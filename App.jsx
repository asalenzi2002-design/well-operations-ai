...
import DNDetailsModal from "@/components/DNDetailsModal";
...
function getStatusTone(status) {
  ...
  if (s.includes("high") || s.includes("shut") || s.includes("overdue") || s.includes("blocked")) return "destructive";
...
function normalizeRisk(raw) {
  if (!raw || typeof raw !== "object") return fallbackRisk;
  return {
    ...fallbackRisk,
    ...raw,
    top_risk_wells: Array.isArray(raw.top_risk_wells) && raw.top_risk_wells.length ? raw.top_risk_wells : fallbackRisk.top_risk_wells,
  };
}

function normalizeIntelligence(raw) {
  if (!raw || typeof raw !== "object") return fallbackIntelligence;
  return {
    ...fallbackIntelligence,
    ...raw,
    production: { ...fallbackIntelligence.production, ...(raw.production || {}) },
    dn: {
      ...fallbackIntelligence.dn,
      ...(raw.dn || {}),
      top_items: Array.isArray(raw?.dn?.top_items) && raw.dn.top_items.length ? raw.dn.top_items : fallbackIntelligence.dn.top_items,
    },
    risk: { ...fallbackIntelligence.risk, ...(raw.risk || {}) },
    drop: { ...fallbackIntelligence.drop, ...(raw.drop || {}) },
    flags: Array.isArray(raw.flags) && raw.flags.length ? raw.flags : fallbackIntelligence.flags,
  };
}
...
export default function App() {
  ...
  const loadData = async (silent = false) => {
    try {
      ...
      setRisk(normalizeRisk(riskData));
      setIntelligence(normalizeIntelligence(intelligenceData));
  ...
  const fieldData = useMemo(() => {
    ...
    return source.map((item) => {
      const rawName = item.field || item.field_code || "Unknown";
      const fieldCode = deriveFieldCode(rawName);
      return {
        name: getFieldLabel(rawName),
  ...
  const normalizedDNItems = useMemo(() => {
    return (intelligence.dn?.top_items || []).map((item) => {
      const progress = Number(item.progress_percent || 0);
      const overdue = progress < 100 && String(item.priority || "").toLowerCase().includes("high");
      const blocked = progress > 0 && progress < 40;
      return {
        ...item,
        progress_percent: progress,
        overdue,
        blocked,
        priority_severity: String(item.priority || "medium").toUpperCase(),
      };
    });
  }, [intelligence]);

  const linkedDNMap = useMemo(() => {
    const map = new Map();
    normalizedDNItems.forEach((item) => {
      const key = item.well_name || "Unknown";
  ...
  const filteredRiskWells = useMemo(() => {
    ...
    const enriched = items.map((item) => ({
      ...item,
      field_code: deriveFieldCode(item.well_name),
      field_label: getFieldLabel(item.well_name),
      linked_dns: linkedDNMap.get(item.well_name) || [],
    }));
    if (!q) return enriched;
    return enriched.filter((item) =>
      [item.well_name, item.production_status, item.field_label, ...(item.reasons || [])]
        .join(" ")
        .toLowerCase()
        .includes(q)
    );
  }, [risk, wellSearch, linkedDNMap]);
  ...
  const fieldDetails = useMemo(() => {
    const productionBreakdown = intelligence.production?.field_breakdown || [];
    return fieldData.map((item) => {
      const code = item.field_code;
      const matchingFieldProduction = productionBreakdown.find((entry) => deriveFieldCode(entry.field_code) === code);
      const highRiskWells = filteredRiskWells.filter((well) => deriveFieldCode(well.well_name) === code);
      const fieldDNs = normalizedDNItems.filter((dn) => deriveFieldCode(dn.well_name) === code);
      const dnPressure = fieldDNs.length;
      const insight =
        highRiskWells.length > 0
          ? `${item.name} is carrying ${formatNumber(item.value)} BOPD with ${highRiskWells.length} high-risk wells and ${dnPressure} DN pressure points.`
          : `${item.name} is contributing ${formatNumber(item.value)} BOPD with limited visible risk pressure right now.`;
      return {
        field_code: code,
        field_name: item.name,
        total_production: item.value,
        well_count: matchingFieldProduction?.well_count || 0,
        high_risk_wells: highRiskWells,
        dn_pressure: dnPressure,
        insight,
      };
    });
  }, [fieldData, intelligence, filteredRiskWells, normalizedDNItems]);
          ...
          <Card className="rounded-2xl">
            ...
            <CardContent className="space-y-3">
              {(intelligence.flags || []).length ? (
                ...
                <EmptyState title="No active flags" description="No current flags were returned by the intelligence layer." />
              )}
            </CardContent>
          </Card>
        </section>
            ...
            <Card className="rounded-2xl">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid gap-3">
                  {filteredRiskWells.length ? (
                    filteredRiskWells.map((well, index) => (
                      <button
                        key={`${well.well_name}-${index}`}
                        type="button"
                        onClick={() => setSelectedWell(well)}
                        className="rounded-2xl border p-4 text-left transition hover:bg-muted/40"
                      >
                        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <Badge variant="outline">{well.field_label}</Badge>
                            </div>
                            <p className="mt-1 text-sm text-muted-foreground">
                    ...
                    <EmptyState title="No wells found" description="No wells matched the current search or no risk wells were returned." />
                  )}
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <ScrollArea className="h-[340px] pr-3">
                  <div className="space-y-3">
                    {normalizedDNItems.length ? (
                      normalizedDNItems.map((item, index) => (
                        <button
                          key={`${item.dn_id}-${index}`}
                          type="button"
                          onClick={() => setSelectedDN(item)}
                          ...
                          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                            <div>
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                {item.blocked ? <Badge variant="secondary">Blocked</Badge> : null}
                              </div>
                      ...
                      <EmptyState title="No DN items" description="The intelligence layer did not return top DN items." />
                    )}
                  </div>
                </ScrollArea>
              </CardContent>
                  ...
                  {fieldData.length ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={fieldData}>
                        ...
                        <YAxis />
                        <Tooltip />
                        <Bar dataKey="value" radius={[10, 10, 0, 0]} onClick={(payload) => payload?.activePayload?.[0]?.payload && setSelectedField(payload.activePayload[0].payload)} />
                      </BarChart>
                    </ResponsiveContainer>
                  ) : (
                    <EmptyState title="No field data" description="No field contribution data is available right now." />
                  )}
                </CardContent>
              </Card>
                    ...
                    fieldDetails.map((item, index) => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          <p className="font-semibold">{item.field_name}</p>
                          <Badge variant="outline">{formatNumber(item.well_count)} wells</Badge>
                        </div>
                        <p className="mt-2 text-sm text-muted-foreground">
                          {formatNumber(item.total_production)} BOPD total contribution
                        </p>
                        <p className="mt-2 text-xs text-muted-foreground">
                          {formatNumber(item.high_risk_wells.length)} high-risk wells • {formatNumber(item.dn_pressure)} DN pressure
                        </p>
                      </button>
                    ))
                  ) : (
                    <EmptyState title="No field breakdown" description="The intelligence layer did not return field breakdown data." />
                  )}
                </CardContent>
              </Card>
            </div>
